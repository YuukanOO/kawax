...
class BuildContext {
  ...
  process(pattern, callback, next) {
    glob(pattern, {
      ...
      const items = matches.map(o => new BuildItem(this, o));
...
`);
      ...
      // Prepare waterfall callbacks and sort them by their function names
      const pluginsSorted = Object.keys(this.plugins)
        .sort()
        .map(o => (cb) => {
          console.info(`Applying ${chalk.blue(o)}`);
          this.plugins[o](this, cb);
        });

      return async.waterfall(pluginsSorted, next);
    });
  }
